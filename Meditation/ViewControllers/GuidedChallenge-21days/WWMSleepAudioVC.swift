...
extension WWMSleepAudioVC: SKProductsRequestDelegate,SKPaymentTransactionObserver{
    ...
    @objc func btnAnnuallyAction(_ sender: Any){
        ...
        for index in 0..<self.productsArray.count {
            ...
            if product.productIdentifier == "get_42_gbp_annual_sub" {
                ...
                Logger.shared.generateLogs(type: "Annually", user_id: self.appPreference.getUserID(), filePath: #file, line: #line, column: #column, function: #function, logText: "selected index... \(self.selectedProductIndex)")
            }
    ...
    func getSubscriptionPlanId(){
        ...
        WWMWebServices.requestAPIWithBody(param: [:], urlString: URL_GETSUBSCRIPTIONPPLANS, context: "WWMSleepAudioVC", headerType: kGETHeader, isUserToken: false) { (response, error, sucess) in
            if sucess {
    ...
    func subscriptionSucessAPI(param : [String : Any]) {
        ...
        WWMWebServices.requestAPIWithBody(param: param, urlString: URL_SUBSCRIPTIONPURCHASE, context: "WWMSleepAudioVC", headerType: kPOSTHeader, isUserToken: true) { (response, error, sucess) in
            if sucess {
