        ...
        for i in 0 ..< dataSets.count
        {
          ...
          var circleRadius = dataSet.circleRadius
            ...
            for j in _xBounds
            {
              ...
              if let selected = (dataProvider as? LineChartView)?.leftAxis.selected, e.y == selected.y, e.x == selected.x{
                circleRadius *= 2
                circleDiameter *= 2
                
                circleHoleRadius *= 2
                circleHoleDiameter *= 2
              }else{
                ...
                if let inverted = (dataSet as? LineChartDataSet)?.invertColors , inverted{
