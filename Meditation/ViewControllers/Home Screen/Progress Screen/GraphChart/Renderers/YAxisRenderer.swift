        ...
        textAlign: NSTextAlignment)
    {
      ...
      let mid = (from + to - 1) / 2
        ...
        for i in stride(from: from, to: to, by: 1)
        {
          ...
          let index = yAxis.getIforindex(i)
          
          let color: UIColor = (yAxis.valueFormatter as? LineChartFormatter)?.getStrip(for: index) ?? UIColor.red
          
          ChartUtils.drawDash(context: context, point: CGPoint(x: fixedPosition - 10, y: positions[i].y + offset + 7), color: color)
          
          if !(i == from || i == (to - 1) || i == mid || (yAxis.selected?.y ?? 0) == index)  {
            continue
          }
          
            if (yAxis.selected?.y ?? 0) == index{
                labelFont = UIFont.boldSystemFont(ofSize: labelFont.pointSize + 4)
            }else{
            ...
            ChartUtils.drawText(
                context: context,
                text: text,
                point: CGPoint(x: fixedPosition - 15, y: positions[i].y + offset),
                align: textAlign,
