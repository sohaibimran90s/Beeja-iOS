...
extension WWMGuidedAudioListVC: SKProductsRequestDelegate,SKPaymentTransactionObserver{
    ...
    @objc func btnAnnuallyAction(_ sender: Any){
        ...
        for index in 0..<self.productsArray.count {
            ...
            if product.productIdentifier == "get_42_gbp_annual_sub" {
                ...
                Logger.logger.generateLogs(type: "Monthly", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "selected index... \(self.selectedProductIndex)")
            }
    ...
    @objc func btnLifeTimeAction(_ sender: Any){
        ...
        for index in 0..<self.productsArray.count {
            ...
            if product.productIdentifier == "get_240_gbp_lifetime_sub" {
                ...
                Logger.logger.generateLogs(type: "LifeTime", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "selected index... \(self.selectedProductIndex)")
            }
            //print(product.productIdentifier)
        }
    }
    ...
    @objc func btnRestoreAction(_ sender: Any){
        Logger.logger.generateLogs(type: "Restore", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "selected index... \(self.selectedProductIndex)")
    ...
    @objc func btnContinueAction(_ sender: Any){
        if self.boolGetIndex{
            ...
            Logger.logger.generateLogs(type: "Continue", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "selected index... \(self.selectedProductIndex)")
            ...
            if reachable.isConnectedToNetwork() {
                ...
                Logger.logger.generateLogs(type: "Continue error", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "lost internet")
            }
        }
    }
    ...
    func requestProductInfo() {
        if SKPaymentQueue.canMakePayments() {
            ...
            Logger.logger.generateLogs(type: "Request Product Info", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "productRequest…… \(productRequest)")
        }
        else {
            //print("Cannot perform In App Purchases.")
            Logger.logger.generateLogs(type: "Request Product Info", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "Cannot perform In App Purchases")
        }
    }
    ...
    func showActions() {
        if transactionInProgress {
            ...
            Logger.logger.generateLogs(type: "Transaction In Progress", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "transaction in progress return")
            return
        }
    ...
    @IBAction func btnDoneAction(_ sender: Any) {
        if  self.productsArray.count > 0 {
            ...
            Logger.logger.generateLogs(type: "Purchase", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "selected product... \(self.productsArray[self.selectedProductIndex])")
    ...
    func productsRequest(_ request: SKProductsRequest, didReceive response: SKProductsResponse) {
        if response.products.count != 0 {
            for product in response.products {
                ...
                //print(product.productIdentifier)
                Logger.logger.generateLogs(type: "SKProductsRequest", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "productsArray... \(productsArray)")
            }
        }
        else {
            Logger.logger.generateLogs(type: "Error: SKProductsRequest", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "There are no products.")
            //print("There are no products.")
        }
        
        if response.invalidProductIdentifiers.count != 0 {
            //print(response.invalidProductIdentifiers.description)
            Logger.logger.generateLogs(type: "SKProductsRequest", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "\(response.invalidProductIdentifiers.description)")
        }
    ...
    func paymentQueue(_ queue: SKPaymentQueue, updatedTransactions transactions: [SKPaymentTransaction]) {
        for transaction in transactions {
            ...
            case SKPaymentTransactionState.purchased, .restored:
                ...
                Logger.logger.generateLogs(type: "Updated Transactions", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "subscriptionPlan \(subscriptionPlan) plan_id \(plan_id)")
                ...
                Logger.logger.generateLogs(type: "Success: Updated Transactions", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "plan_id \(param)")
            ...
            case SKPaymentTransactionState.failed:
                ...
                Logger.logger.generateLogs(type: "Error: Transaction State Failed", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "transaction failed")
                
            default:
                //print(transaction.transactionState.rawValue)
                Logger.logger.generateLogs(type: "Default: Transaction State", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "transaction in default \(transaction.transactionState.rawValue)")
            }
        }
    }
        ...
        WWMWebServices.requestAPIWithBody(param: [:], urlString: URL_GETSUBSCRIPTIONPPLANS, context: "WWMUpgradeBeejaVC", headerType: kGETHeader, isUserToken: false) { (response, error, sucess) in
            if sucess {
                if let result = response["result"] as? [[String: Any]]{
                    ...
                    Logger.logger.generateLogs(type: "Success: getSubscriptionPlanId", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "responseArray \(self.responseArray)")
                   //print("result.... \(result)")
                }
            }else {
    ...
    func subscriptionSucessAPI(param : [String : Any]) {
        ...
        WWMWebServices.requestAPIWithBody(param: param, urlString: URL_SUBSCRIPTIONPURCHASE, context: "WWMUpgradeBeejaVC", headerType: kPOSTHeader, isUserToken: true) { (response, error, sucess) in
            if sucess {
                ...
                Logger.logger.generateLogs(type: "Success: SubscriptionSucessAPI", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "param \(param)")
                
                if self.continueRestoreValue == "1"{
                    
                    Logger.logger.generateLogs(type: "Success: continueRestoreValue: 1", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "param \(param)")
                    ...
                    Logger.logger.generateLogs(type: "Success: continueRestoreValue: 0", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "param \(param)")
                ...
                if error != nil {
                    ...
                    Logger.logger.generateLogs(type: "Fail: SubscriptionSucessAPI", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "error?.localizedDescription \(error?.localizedDescription ?? "")")
                ...
                Logger.logger.generateLogs(type: "Fail: SubscriptionSucessAPI", user_id: self.appPreference.getUserID(), filePath: "\(self)", line: #line, column: #column, function: #function, logText: "error)")
            }
            //WWMHelperClass.dismissSVHud()
            WWMHelperClass.hideLoaderAnimate(on: self.view)
        }
